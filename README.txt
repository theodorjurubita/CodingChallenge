...
2. How to run it
	...
	C. Run the program's specific commands using the following template: >dotnet run [arguments]
		...
		*Please note that the commands should be written on a single line as it may cause incorrect read of arguments.
	D. Reading from a json file (e.g. >dotnet --read-file transactions.json) is possible if the file transactions.json exists inside the csproj folder.
		*Please note that reading from file was tested only on Windows, so it might not function well with other file systems of other operating systems.
...
3. Notes
	...
	C. The program is inteded to handle only the commands specified in the pdf provided. Simpler approaches were chosen for brevity.
